"""Script for running the final model - the linear-bilinear model - on training and test data. 
...
# Custom modules
from modules.preprocessing import preprocess, get_data_from_source, get_sentences
...
from modules.modelling.models.neuralnetworks import LinearBilinearModel
...
def main(args):
    ...
    assert Path(args.train_data).exists() and Path(args.val_data).exists() and Path(args.test_data).exists(), f"Data path given does not exist."
    ...
    # # Load data
    train_data = get_data_from_source(args.train_data) + get_data_from_source(args.val_data)
    test_data = get_data_from_source(args.test_data)
    
    ## Train tensor representations on train data
    # Get corpus training
    tensor_corpus = get_sentences(train_data)
    ...
    X_test, y_test = preprocess(test_data, tensorizer)
    ...
    # # Train model
    model = LinearBilinearModel(len(context_base), args.hidden_dim, args.dropout)
    optim = torch.optim.Adam(model.parameters(), args.lr)
    loss = torch.nn.BCELoss()
    acc = lambda true, pred: (true == pred).sum().item() / len(true)
    trainer = NNTrainer(model, optim, loss, acc, 3)
    
    trainer.fit(X_train, y_train, X_test, y_test, args.epochs, args.eval_every, args.batch, args.verbose)
...
def arg_parser() -> 'argparse.Namespace':
    ...
    parser = argparse.ArgumentParser(
        ...
        '''
description:
    Script for running the final model - the linear-bilinear model - on training and test data. Saves the fitting history in the destionation path provided.
    
example:
    python .\model_scripts\linear_bilinear_training.py 
        --train_data .\data\data_train.txt 
        --val_data .\data\data_val.txt
        --test_data .\data\data_test.txt
    ...
    parser.add_argument(
        '--test_data',
        metavar='<file_path>.txt',
        help='Specify the test data path. Required.',
        type=str,
        required=True
    )
    ...
    parser.add_argument(
        '--dropout',
        metavar='<float>',
        help='Specify the probability of dropout for the hidden layers. Defaults to 0.5.',
        type=float,
        default=0.5
    )
    ...
    parser.add_argument(
        '--hidden_dim',
        metavar='<int>',
        help='Specify the size of the hidden layer. Defaults to 1.',
        type=int,
        default=1
    )
